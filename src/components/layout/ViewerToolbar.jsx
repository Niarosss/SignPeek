...
export default function ViewerToolbar({ file, onRemove }) {
  return (
    <div className="h-12 md:h-14 border-b border-slate-200 bg-white flex items-center justify-between px-2 md:px-6 shrink-0 z-10">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <a 
          href={file.url} 
          download={file.name}
          className="flex items-center gap-2 bg-slate-800 text-white px-2 md:px-4 py-2 rounded-xl text-xs font-bold hover:bg-slate-600 transition-all shadow-sm active:scale-95"
        >
          <DownloadSimpleIcon size={16} weight="bold" />
          <span className="hidden md:inline uppercase tracking-wider">
            Зберегти {file.extension?.toUpperCase()}
          </span>
        </a>
